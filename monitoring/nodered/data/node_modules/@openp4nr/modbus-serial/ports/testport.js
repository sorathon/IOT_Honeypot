...
class TestPort extends EventEmitter {
    ...
    write(data, argumentMap) {
        ...
        if(functionCode === 20) {
            ...
            buffer.writeUInt8(0x05, 3);
            buffer.writeUInt8(0x06, 4);
            buffer.writeUInt8(0x0D, 5);
            buffer.writeUInt8(0xFE, 6);
            buffer.writeUInt8(0x00, 7);
            buffer.writeUInt8(0x20, 8);
        }
        ...
        if (buffer) {
            ...
            switch (unitNumber) {
                ...
                case 6:
                    ...
                    return;
                case 7: {
                    // unit 3: answers that are bigger than expected
                    const tempCopy = Buffer.alloc(buffer.length);
                    buffer.copy(tempCopy, 0, 0, buffer.length);
                    buffer = Buffer.alloc(tempCopy.length + 4);
                    tempCopy.copy(buffer, 0, 0, tempCopy.length);
                    break;
                }
            }
