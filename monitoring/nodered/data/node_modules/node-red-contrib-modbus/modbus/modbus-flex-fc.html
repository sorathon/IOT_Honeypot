...
placeholder='/home/username/codes.json'> <button id='node-input-loadMap' type='button' class='red-ui-button'>Load
...
Select Map</option> </select> <button id='node-input-reset-request' type='button' class='red-ui-button'>Reset Maps
...
Unit-ID - The Device Identification the Node tries to reach via MODBUS.</p> <p>
 Supply a JSON Custom Function Code map file in the <strong>Path to Map JSON</strong>
 textbox then press the load button the editor will then try to load the given file. <strong>NOTE:</strong>
 If you want to specify a local file that is not relative to your current directory, you need to make sure that your path is 
<strong>absolute</strong> and not relative. (i.e. <strong>"/home/username/codes/codes.json"</strong> will work while 
<strong>"./codes/codes.json"</strong>
 would not). You can name your codes.json file whatever you want just make sure that it follows the specified layout below or take a look at the examples that are coming with the packages (extras/argumentMaps/defaults/codes.json) 
</p> <p>
...
<strong>names,values, offsets or types</strong> of the arguments. </p> <p><strong>Request Map Editor -</strong>
 ...
 Data editor for the response of the custom function code.</p> <p> layout of an argument map: </p><ul> <li><strong>
Name -</strong> The Name of the Argument. On the <strong>Request side</strong>
